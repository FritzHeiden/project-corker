...
var _webServer = __webpack_require__(52);

var _webServer2 = _interopRequireDefault(_webServer);

var _fileHandler = __webpack_require__(120);

var _fileHandler2 = _interopRequireDefault(_fileHandler);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var AppPath = exports.AppPath = _path2.default.dirname(process.mainModule.filename);
var port = 2345;

var webServer = new _webServer2.default(port);
var fileHandler = new _fileHandler2.default();
...
var WebServer = function () {
    function WebServer(port) {
        ...
        // Serve Web App
        this.app.use(_express2.default.static(_path2.default.join(_core.AppPath, "./web-app")));
    }
    ...
    _createClass(WebServer, [{
        ...
        value: function addHandler(handler) {
            ...
            handler.getHandlers().forEach(function (handler) {
                _this.handle(handler.method, handler.path, handler.callback);
            });
        }
    }, {
        key: 'handle',
        value: function handle(method, path, callback) {
        ...
        value: function start() {
            ...
            this.app.listen(this.port, function () {
                return console.log("Server started on port " + _this2.port);
            });
        }
    }]);
...
var FileHandler = function () {
    ...
    _createClass(FileHandler, [{
        ...
        value: function _listDir(dir) {
            ...
            return new Promise(function (resolve, reject) {
                ...
                getFiles.then(function (files) {
                    var getStats = Promise.all(files.map(function (file) {
                        return _this._getStats(file);
                    }));

                    getStats.then(function (stats) {
                        var responseFiles = { directories: [], files: [] };
        ...
        value: function getHandlers() {
            return [{ method: "get", path: "/api/files", callback: this._getFiles.bind(this) }, { method: "get", path: "/api/file", callback: this._getFile.bind(this) }];
        }
    }], [{
        key: '_getPath',
        value: function _getPath(request) {
            var requestPath = "";
            var absolute = request.query.absolute ? request.query.absolute === "true" : false;

            if (request.query.path) {
                requestPath = request.query.path;
                if (!absolute) {
                    requestPath = _path2.default.join(_core.AppPath, requestPath);
                }
            } else {
                requestPath = "/";
                if (!absolute) {
